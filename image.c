...
// tablica kolorów - po 4 wartości na kolor - 4 koljene liczby to zapis rgb+maska
...
void set_white_4bpp(ImageInfo* pImg)
{
    memset(pImg->pImg, 0xFF, pImg->line_bytes * pImg->height);
}

void multiplyLine(ImageInfo* pImg) 
{
    for (unsigned int i = 1; i < pImg->height; i++) 
    {
        unsigned char* targetPtr = pImg->pImg + i * pImg->line_bytes;
        memcpy(targetPtr, pImg->pImg, pImg->line_bytes);
    }
}
